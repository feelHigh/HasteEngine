									   ...
									   , DXGI_FORMAT _Format, UINT _Flags, D3D11_USAGE _Usage)
{
	...
	{
		MessageBox(nullptr, L"Texture(Type0) creation failed.", L"텍스쳐 생성 실패", MB_OK);
		return nullptr;
	}
...
Ptr<CTexture> CAssetMgr::CreateTexture(wstring _strKey, ComPtr<ID3D11Texture2D> _Tex2D)
{
	...
	{
		MessageBox(nullptr, L"Texture(Type1) creation failed.", L"텍스쳐 생성 실패", MB_OK);
		return nullptr;
	}
...
void CAssetMgr::DeleteAsset(ASSET_TYPE _Type, const wstring& _Key)
{
	...
	// Asset 변경 알림
	CTaskMgr::GetInstance()->AddTask(tTask{ ASSET_CHANGED });
}
